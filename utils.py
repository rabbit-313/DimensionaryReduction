...
# mvtype = ['bottle', 'cable', 'capsule', 'carpet', 'grid', 'hazelnut', 'leather','metal_nut', 'pill', 'screw', 'tile', 'toothbrush', 'transistor', 'wood', 'zipper']

corruption_tuple = ("gaussian_noise", "shot_noise", "impulse_noise", "defocus_blur","glass_blur", "motion_blur", "zoom_blur", "snow", "frost", "fog",
...
def get_resnet_model(resnet_type=152, pretrained=True):
    ...
    """
    if resnet_type == 18:
        return ResNetYoda.resnet18(pretrained=pretrained, progress=True)
    elif resnet_type == 50:
        return ResNetYoda.wide_resnet50_2(pretrained=pretrained, progress=True)
    elif resnet_type == 101:
        return ResNetYoda.resnet101(pretrained=pretrained, progress=True)
    else:
        return ResNetYoda.resnet152(pretrained=pretrained, progress=True)
...
class CustomDataset(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        for key, value in self.idx_dict.items():
            ...
            imgs_and_labels[key] = img, domain_label
